...
This repository is a Next.js site in `site/` managed with pnpm, linted and formatted with Biome, and orchestrated through `just`.
...
| --- | --- |
| `just default` | Lists all available recipes |
| `just install` | Installs site dependencies with pnpm |
| `just format` | Formats and auto-fixes code with Biome |
...
| `just update` | Updates dependencies with pnpm |
| `just build` | Builds the site for production |
| `just run-dev-server` | Starts the Next.js development server |
| `just run-prod-server` | Builds the site and starts it in production mode |

1. Fork the repository and clone your fork locally
2. Create a branch following the [branch naming convention](#branches)
3. Run `just install`, make your changes, then run `just check` before opening a pull request
